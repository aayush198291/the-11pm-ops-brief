...
def init_lakebase_config() -> Optional[LakebaseConfig]:
    ...
    if not endpoint and not raw_name and not has_autoscaling:
        logger.info(
            "No Lakebase env vars set — long-term memory disabled, short-term memory uses InMemorySaver."
        )
        return None
...
async def run_lakebase_setup(config: Optional[LakebaseConfig]) -> None:
    """Run database migrations for checkpoint and store tables. Call once at app startup."""
    if config is None:
        logger.info("Lakebase config is None — skipping setup; memory will use InMemorySaver.")
        return
